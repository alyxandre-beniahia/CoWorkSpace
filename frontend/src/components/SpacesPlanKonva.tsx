...
export function SpacesPlanKonva({ onSelectSpace, editable = false, onPositionChange }: SpacesPlanKonvaProps) {
  const { token } = useAuth()
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Vue simplifiée des postes et salles pour aujourd&apos;hui</p>
        </div>
      </div>
...
function GroupSpace({ x, y, width, height, space, bgColor, onClick, editable = false, onPositionChange }: GroupSpaceProps) {
  return (
    <>
      <Rect
        x={x}
