...
describe('SetMemberActiveUseCase', () => {
  ...
  it('lance AdminNotFoundError si userId inexistant', async () => {
    await expect(useCase.run('unknown-id', true)).rejects.toThrow(AdminNotFoundError);
  });

  it("lance AdminForbiddenError si user n'est pas member (ex. admin)", async () => {
    await expect(useCase.run(adminUserId, false)).rejects.toThrow(AdminForbiddenError);
  });
});
