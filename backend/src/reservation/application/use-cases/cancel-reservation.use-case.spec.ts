...
describe('CancelReservationUseCase', () => {
  ...
  it('annule une réservation avec succès (soft delete)', async () => {
    const result = await useCase.run(reservationId, userId, 'this');
  ...
  it('lance ReservationNotFoundError si réservation inexistante', async () => {
    await expect(useCase.run('unknown-id', userId)).rejects.toThrow(ReservationNotFoundError);
  });

  it("lance ReservationForbiddenError si l'utilisateur ne possède pas la réservation", async () => {
    await expect(useCase.run(reservationId, otherUserId)).rejects.toThrow(
      ReservationForbiddenError,
    );
  });
});
