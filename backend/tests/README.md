...
| Causes des échecs | Aucune : après mise à jour du schéma Prisma et de la base de données (enum `NotificationType` migré), toutes les écritures dans `notificationLog` sont acceptées. |
| Date | Voir la date d’exécution dans le rapport Jest généré localement. |
| Responsable des tests | LADMIA Ryan. |
| Objectifs cibles | 100 % des suites et des tests doivent rester verts à chaque exécution CI et en local avant livraison. |
